...
if [[ "${SKIP_SOURCE}" == "no" ]]; then
  ...
  echo "LOOPHOLE_VERSION=\"${LOOPHOLE_VERSION}\"" >> dev/build.env
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ "${OS_NAME}" == "osx" && -f "./build/osx/include.gypi" ]]; then
...
if [[ "${SKIP_ASSETS}" == "no" ]]; then
  ...
  if [[ "${OS_NAME}" == "osx" && -f "dev/osx/codesign.env" ]]; then
    ...
    echo "Loaded macOS signing configuration"
  fi
